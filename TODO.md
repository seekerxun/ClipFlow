...
四项交互已接：片段循环（A-B）、播放区滚轮调音量、`Z`/`X`/`C` 调倍速且设封面改为 `B`、拖入与打开改为加入列表（多文件夹 / 多视频混合、路径去重），列表不再显示单一目录名。

启动默认空列表，不恢复上次素材；窗口布局仍记住。目录变化监听与排序/搜索已可手测。预载与分发脚本仍未做。

手测：冷启动首屏约 2–3 秒、二次秒开，所有者已验收。

起始百分比已接参数和 UserDefaults（默认 35%），设置界面还没有。
