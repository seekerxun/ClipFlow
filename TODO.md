...
| └ **5. 播放** | ✅ 完成（画面能嵌进 SwiftUI；Esc 退出全屏已接） |
| └ **6. 界面与输入** | ✅ 完成（快捷键 / 控制条 / B 键 / 循环 / 铺满窗口已接） |
...
- [x] “播放”菜单 / `H` 切换水平翻转，兼容 VideoToolbox 硬解帧且不影响其他滤镜
- [x] 循环模式：单个循环 / 列表循环 / 关闭；单文件列表也能列表循环
...
- [x] 在 Finder 里多选几个视频一起打开（或用命令行一次带上多个文件）时，打开几个就会弹出几扇窗口。所有文件都进了最前面那一扇，那扇能正常播放，其余几扇是空的。
- [x] 已经在一次打开 3 个和 5 个文件时复现。冷启动只打开一个文件、以及不带文件直接启动都正常，都只有一扇窗口。
- [x] 起因在系统的多窗口机制：每个打开文件的事件都会被当成要新开一扇窗口，应用这边只来得及把文件都收进最前面那扇。

已修复。系统送「打开文件」时是一个文件一条事件，而系统默认的处理会为每条事件各开
一扇窗口——这一步在应用自己的代码跑起来之前就已经做完了，所以之前怎么调整都拦不住。
现在应用把这条事件接管过来自己处理，系统那套「一个文件一扇窗」就不再执行；
陆续送来的文件先攒成一批，再统一决定放进哪扇窗口，一扇都没有时才开一扇新的。

顺带修掉一个一直都在的小毛病：窗口标题偶尔停在应用名而不是正在播的文件名
（原先绕过 SwiftUI 直接改窗口，改早了会被 SwiftUI 再盖回去），现在标题交给
SwiftUI 自己写。

已验证：不带文件启动、冷启动开 1 / 3 / 5 个文件、已有空窗口时再开、只有非空窗口时
再开、关掉最后一扇，各 6 次全部符合预期；一次开 30 个文件只有一扇窗口、30 个文件
全在列表里、只有一个进程。

**完成标准：** 一次打开多个文件只留一扇窗口，文件全部落在这扇窗口的列表里，不再出现空窗口。

补了一层兜底。上面这套「一扇窗口都没有时开出第一扇」用的开窗能力，是菜单建好时
顺手记下来的，而记它的那一步系统并不保证一定会跑。真没跑的话，文件会一直躺在等待
队列里，屏幕上既没有窗口也没有报错——文件看起来就是凭空消失了。出错的概率很低，
但一出错就是文件不见，所以现在改成：拿不到那份能力就自己直接开一扇窗口；拿到了也
盯一小会儿，叫了却没开出窗口的话同样自己补开。攒批的规矩没变，一次开多个文件仍然
只会开一扇。

顺带清掉一处隐患：「再开一扇窗口」的能力原先是一个全局的记号，窗口关掉也不清，
于是「把窗口全关掉再打开文件」这条路走的是一扇已经拆掉的窗口留下的旧记号。
现在它跟着窗口一起登记、一起注销。

#### 5. 文件夹拖到程序坞图标上没反应

- [x] 单个视频、多个视频拖到程序坞图标上都正常，只有文件夹不行。
- [x] 不是接管「打开文件」事件之后才坏的，一直就没支持过：应用只对外声明了自己能开
      视频文件，没声明能开文件夹，系统因此根本不会把文件夹送过来——程序坞图标不亮，
      访达的「打开方式」里也找不到片巡。
- [x] 代码这一侧一直是收得下文件夹的（和菜单里的「打开文件夹…」走同一条路），
      补上这份声明即可。仍然只当备选打开方式，访达继续是文件夹的默认打开方式。

已验证：新构建在系统里已经是文件夹的可选打开方式；把一个含两个视频的文件夹交给
应用，会新开一扇窗口并把里面的视频都扫进列表。

**注意：** 这份声明写在应用自己的信息里，要等新版本装到 `/应用程序` 并被系统重新
登记之后，程序坞图标才会接受文件夹。用旧版本试不出效果。

**完成标准：** 文件夹拖到程序坞图标上能打开，里面的视频进入列表，和菜单里的
「打开文件夹…」结果一致。
...
| 索引实例 | **全应用只有一份**（`MediaIndex.shared`）。索引是整机素材缓存，不属于任何一扇窗口；每扇窗口各建一个会让落盘互相覆盖，记录凭空变少 |
| 索引落盘 | **先读磁盘再合并再写**。整份覆盖只要有第二个写入方就会抹掉自己没见过的记录；合并之后这件事在结构上不可能发生 |
