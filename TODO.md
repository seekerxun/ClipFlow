...
| **V1.1** 高效浏览 | 🔵 进行中。网格 / 列表切换 / 拖出已齐；SeekPreview 未做；完成标准未手测 |
| **V2 / 暂缓** | ⬜ 未开始 |
...
V1 可用已验收。V1.1 网格 / 切换 / 拖出已落地，下一步是 SeekPreview。

手测：冷启动首屏约 2–3 秒、二次秒开，所有者已验收。

起始百分比已接参数和 UserDefaults（默认 35%），设置界面还没有。V1.1 还剩进度条 hover 预览（SeekPreview）。
