...
正在做四件事（均未完成）：片段循环（A-B）、播放区滚轮调音量、`Z`/`X`/`C` 调倍速且设封面改为 `B`、拖入单个或多个视频文件。
...
- [ ] 片段循环（A-B）：`[` / `]` 把当前播放位置设为起点 / 终点；Shift+`[` / Shift+`]` 取消。进度条画出 A/B 标记，两点之间高亮。两端都有且起点早于终点才循环；只设一端不循环；终点早于起点则对调。有 A-B 时播到 B 回到 A，不跳下一条；清掉后 `L` 恢复原意
- [ ] 播放区鼠标滚轮调音量（上大下小）；素材列表滚轮仍滚动列表
- [ ] 倍速：`Z` 循环减速 0.75× → 0.5× → 0.25× → 1×；`X` 恢复 1×；`C` 循环加速 1.25× → 1.5× → 2× → 1×。设封面改为 `B`（不再用 `C`）
- [ ] 拖入：除文件夹外，也可拖入单个或多个视频文件
