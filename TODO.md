...
- [ ] 在 Finder 里多选几个视频一起打开（或用命令行一次带上多个文件）时，打开几个就会弹出几扇窗口。所有文件都进了最前面那一扇，那扇能正常播放，其余几扇是空的。
- [ ] 已经在一次打开 3 个和 5 个文件时复现。冷启动只打开一个文件、以及不带文件直接启动都正常，都只有一扇窗口。
- [ ] 起因在系统的多窗口机制：每个打开文件的事件都会被当成要新开一扇窗口，应用这边只来得及把文件都收进最前面那扇。
