...
| **V1** 可用 | ✅ 完成。所有者已验收 |
| └ 1. 扫描与索引 | ✅ 完成 |
| └ 2. 精灵图流水线 | ✅ AVFoundation 主路径完成；ffmpeg 回退列入近期计划 |
...
按优先级处理三项已确认问题：关闭窗口后残留声音、播放中调整窗口 / 全屏卡顿、AVFoundation 不支持的素材没有缩略图。具体步骤和完成标准见「V2 — 打磨」中的近期修复计划。
...
- [ ] 将关闭事件绑定到具体窗口，停止并释放该窗口的 mpv 实例和渲染上下文。
- [ ] 不退出整个应用，不影响其他仍然打开的播放窗口。
...
- [ ] 合并重复画面更新，只处理最新一帧，避免过时绘制排队。
- [ ] 连续缩放时优先保证窗口响应，必要时临时降低视频刷新频率，声音保持连续。
...
- [ ] 元数据主路径失败时，用 ffprobe 获取时长与应用旋转后的显示尺寸。
- [ ] 用 ffmpeg 生成与主路径同规格的封面和 24 帧精灵图，覆盖常见 MKV、AVI、RM、WebM、FLV、WMV、TS 等素材。
- [ ] 单次任务只启动一个进程，不逐帧启动 ffmpeg；保留实际时间映射，供悬停扫过和进度条预览使用。
- [ ] 接入现有超时、取消、并发上限与失败落库，坏文件不能拖住整个队列。
- [ ] 开发阶段不做历史失败记录迁移；复测时清空索引数据和缩略图缓存。
