...
| **V1** 可用 | ✅ 完成。所有者已验收 |
| └ 1. 扫描与索引 | ✅ 完成 |
| └ 2. 精灵图流水线 | ✅ AVFoundation 主路径与 ffprobe / ffmpeg 回退完成 |
| └ 3. 封面帧选取 | ✅ 完成（`B` 键已接；起始百分比默认 35%，尚无设置界面） |
| └ 4. 调度 | ✅ 完成（并发上限、两阶段拆分、可见优先级调度） |
| └ **5. 播放** | ✅ 完成（画面能嵌进 SwiftUI；F / Esc / 中键全屏已接） |
| └ **6. 界面与输入** | ✅ 完成（快捷键 / 控制条 / B 键 / 循环 / 全屏已接） |
| **V1.1** 高效浏览 | ✅ 完成。所有者已验收（网格能判断素材） |
| **V2** | 进行中 / 可手测 |
| **补遗** 播放与导入 | ✅ 完成 |
| **暂缓** | ⬜ 未开始 |
...
三项近期修复均已完成代码与自动测试，下一步是真机验证关窗和缩放 / 全屏体验，并用真实旧格式素材验收缩略图回退。具体剩余项见「V2 — 打磨」。

开发阶段不迁移历史缩略图失败记录；需要复测时清空索引数据和缩略图缓存。预载与分发脚本仍按原条件暂缓。
...
- [x] `FFmpegSpriteGenerator`：AVFoundation 失败后的 ffmpeg 单进程回退
...
- [x] **`B` 键手动覆盖封面**（当前播放时间 + AVFoundation 抽一帧，只重建这一张）
- [ ] 起始百分比做成偏好设置，默认 35%（流水线已接参数与 UserDefaults 默认值，尚无设置界面）

> 逃生口（`B` 键 + 可调百分比）比启发式本身更重要。实测这批 1096 个素材兜底率 0%，但那是因为它们没有片头 logo——真正要解决的场景没能在这批数据上得到验证。
...
- [x] 精灵图全量生成 **< 30s**
- [x] 切换视频 **< 150ms**
- [x] 缩略图墙**没有成片的黑帧和 logo 卡**；少数选不准的，悬停扫过一秒内能看清内容
- [x] 能用键盘从头到尾扫完整个目录，手不离开左手区
...
- [x] `MediaGridView` 网格模式，一屏 40–60 个缩略图（复用 V1 已做好的悬停扫过）
- [x] `SeekPreview`：进度条 hover 缩略图预览，读精灵图（零解码，不需要第二个解码实例）
- [x] 列表 / 网格模式切换
- [x] **拖出**：从列表或网格拖文件到 Premiere / FCP / Finder（`NSItemProvider` 包 file URL，成本极低）

### V1.1 完成标准

- [x] 网格模式下不点击任何一个视频，仅靠悬停扫过就能判断一批素材的内容
- [x] 进度条 hover 预览响应 < 16ms
...
- [x] 将关闭事件绑定到具体窗口，停止并释放该窗口的 mpv 实例和渲染上下文。
- [x] 不退出整个应用，不影响其他仍然打开的播放窗口。
- [ ] 手测普通窗口、全屏窗口和多窗口分别关闭的情况。

**完成标准：** 播放中按 `Cmd+W` 后声音立即停止；再开新窗口时没有旧播放器残留，新窗口和其他已有窗口都能正常播放。

#### 2. 改善播放中的窗口缩放与全屏卡顿

- [ ] 用 1080p 和高码率 4K 素材分别记录缩放、进入全屏和退出全屏时的主线程占用与重绘频率。
- [x] 合并重复画面更新，只处理最新一帧，避免过时绘制排队。
- [x] 连续缩放 / 全屏过渡时约以 30fps 更新画面，结束后补画最新帧；声音连续性待真机确认。
- [ ] 仅当前述优化仍不够时，再评估跟随屏幕刷新节奏或调整渲染线程。

**完成标准：** 播放中连续拖动窗口边缘、进入和退出全屏时，窗口和控制条不明显卡住，声音不中断；全屏动画期间允许少量视频掉帧。

#### 3. 为 AVFoundation 不支持的素材补缩略图回退

- [x] 元数据主路径失败时，用 ffprobe 获取时长与应用旋转后的显示尺寸。
- [x] 用 ffmpeg 生成与主路径同规格的封面、`B` 键指定封面和 24 帧精灵图。
- [x] 单次任务只启动一个进程，不逐帧启动 ffmpeg；保留实际时间映射，供悬停扫过和进度条预览使用。
- [x] 接入现有超时、取消、并发上限与失败落库，坏文件不能拖住整个队列。
- [x] 开发阶段不做历史失败记录迁移；复测时清空索引数据和缩略图缓存。
- [ ] 用 AVFoundation 无法抽帧但 libmpv 能播放的真实 MKV、AVI、RM 素材手测。

残余限制：极低帧率或不足 24 个实际画面的素材会重复最近真实帧；依赖 Homebrew ffmpeg；历史失败记录不会自动迁移。

**完成标准：** MKV、AVI、RM 至少各准备一份 AVFoundation 无法抽帧但 libmpv 能播放的测试素材；首次扫描能显示封面，悬停扫过和进度条预览可用，坏文件不会被无限重试。

### 其余项目

- [x] 递归扫描子目录（**深度上限**，防符号链接成环）—— V1 已有深度 8，未放宽
- [x] FSEvents 监听，目录内新增 / 删除文件自动更新
- [x] 简单排序 / 搜索（按名称、时长、分辨率、日期）
- [x] 记住窗口布局（启动不恢复素材列表）
...
- [x] 片段循环（A-B）：`[` / `]` 把当前播放位置设为起点 / 终点；Shift+`[` / Shift+`]` 取消。进度条画出 A/B 标记，两点之间高亮。两端都有且起点早于终点才循环；只设一端不循环；终点早于起点则对调。有 A-B 时播到 B 回到 A，不跳下一条；清掉后 `L` 恢复原意
- [x] 播放区鼠标滚轮调音量（上大下小）；素材列表滚轮仍滚动列表
- [x] 倍速：`Z` 循环减速 0.75× → 0.5× → 0.25× → 1×；`X` 恢复 1×；`C` 循环加速 1.25× → 1.5× → 2× → 1×。设封面改为 `B`（不再用 `C`）
- [x] 拖入：除文件夹外，也可拖入单个或多个视频文件
- [x] Finder「打开方式」可选片巡；再打开视频时，空窗口就地加入，已有内容则新开窗口；`Cmd+N` 新建窗口
...
| 精灵图抽帧 | 时间偏差限定在取样间隔的一半以内，不只解关键帧；24 帧的实际时间戳存进索引 |
| 封面帧选取 | 时间窗口 [35%, 80%] + 纯色剔除，约 40 行。dHash、跨文件片头聚类、Sobel、色彩丰富度**全部作废** |
| 封面选不准怎么办 | `B` 键手动覆盖 + 起始百分比可调。**不再加码启发式** |
...
| 历史缩略图失败记录 | 开发阶段不做迁移；回退功能完成后清空索引数据与缩略图缓存复测 |
| 关闭窗口 | 只停止并释放该窗口的播放器，不影响其他窗口，应用不必随最后一扇窗口退出 |
