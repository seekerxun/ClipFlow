...
| **V1** 可用 | 🔵 进行中，第 1–5 块完成；第 6 块前半（布局 / 列表 / 悬停扫过 / 单击即播）已完成 |
| └ 1. 扫描与索引 | ✅ 完成 |
| └ 2. 精灵图流水线 | ✅ 完成（ffmpeg 回退路径暂缓） |
| └ 3. 封面帧选取 | ✅ 完成（差 `C` 键与偏好设置，都要等界面） |
| └ 4. 调度 | ✅ 完成（并发上限、两阶段拆分、可见优先级调度） |
| └ **5. 播放** | ✅ 完成（画面能嵌进 SwiftUI；F/Esc 快捷键留给第 6 块后半） |
| └ **6. 界面与输入** | 🔵 **前半完成 ← 下一步：快捷键与控制条** |
| **V1.1 / V2 / 暂缓** | ⬜ 未开始 |
...
第 6 块前半已接上：打开目录、列表、悬停扫过、可见优先级调度、单击即播。接下来做后半：快捷键全集、播放控制条、`C` 键封面、循环 / 自动下一个的键盘、全屏快捷键。做完的判据是「能用键盘从头到尾扫完一个真实目录」，详见 V1 完成标准。
