...
/// 控制条只留下播放、音量、倍速、显示比例、旋转、循环和逐帧时必要的前后帧按钮。
...
/// 播放控制条：播放/暂停、进度、音量/静音、倍速、显示比例、旋转、循环。进度条 hover 读精灵图预览。
        ...
        HStack(spacing: 10) {
            ...
            Button {
                ...
                Image(systemName: "rotate.right")
                    .foregroundStyle(
                        controller.videoRotationDegrees == 0 ? .secondary : Color.accentColor
                    )
                    .frame(width: 30, height: 30)
                    .background(Color.white.opacity(0.055), in: RoundedRectangle(cornerRadius: 7))
            }
            .buttonStyle(.plain)
            .focusable(false)
            .help("顺时针旋转 90°（当前 \(controller.videoRotationDegrees)°）")
