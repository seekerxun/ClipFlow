...
- 控制条只保留播放、逐帧时的前后帧、音量、倍速与循环；显示比例、旋转、水平翻转、逐帧模式和全屏在菜单栏操作
...
- 倍速
- “播放”菜单可覆盖显示比例（原始、1:1、4:3、3:2、16:9、9:16），切换时即时重绘，不重新载入视频、不增加滤镜
- “播放”菜单或 `R` 可使画面顺时针旋转，每次 90°，按 0° / 90° / 180° / 270° 循环，不重新载入视频、不增加滤镜
- “播放”菜单或 `H` 可切换水平翻转；滤镜会兼容 VideoToolbox 硬解帧，不重新载入视频、不影响其他滤镜
- 循环播放（单个循环 / 列表循环 / 关闭）；列表只有一个文件时，列表循环会从头重播该文件；循环模式记住最后一次选择并在下次启动时恢复
...
- 逐帧模式（默认关闭，`G` 或菜单栏“播放”菜单开关）：见下

#### 逐帧模式

用来一帧一帧看某个视频。开启后：

- 立即暂停，进度条改成按帧走，时间读数换成 `帧 123 / 217`（时间挪到读数的悬停提示里）
- `A` / `←` 退一帧，`D` / `→` 进一帧；`W` / `↑` 和 `S` / `↓` 一次十帧
- 控制条上多出上一帧 / 下一帧两个按钮
- 这期间换视频不会自动播，停在第一帧等着看

帧号不按「平均帧率 × 当前时间」算。不少素材是变帧率的——比如 24fps 的内容存在 60fps 的时间轴上——乘出来的帧号会时不时跳一格或者卡住不动（实测一个 217 帧的样片，这么算有 72 帧的号是错的）。开启逐帧模式时会用 ffprobe 把这个文件每一帧的时间读成一张表（只读封装不解码，9 秒的片子约 0.03s），帧号、进度条刻度和跳转都以它为准。表没读到之前先按帧率估，不影响播放。

跳转时的目标时间取「上一帧和这一帧的中点」：播放内核精确定位后停在第一个不早于目标时间的帧上，正好踩在这一帧的时间上会因为浮点误差多走一帧。

模式本身不做记忆，每次启动都是关闭状态；开着的时候换视频仍然保持开着。
...
Delete                   将选中素材移出列表
Shift+Delete             删除选中素材（优先移到废纸篓）
播放区滚轮               调音量（上大下小；素材列表滚轮仍滚动列表）
```

素材列表支持 Shift 连续多选和 Command 增减多选。右键素材可在访达显示鼠标下的单个文件，或对当前多选批量执行“移除列表”“删除”；删除使用红色破坏性菜单项并优先移到废纸篓。SMB 等位置没有废纸篓时，必须在第二次警告中明确确认，才会永久删除且无法恢复。

当前播放项被移除或删除时，有剩余素材则继续播放相邻项；列表已经为空则立即卸载文件、释放文件句柄并回到空列表界面，不保留已删除视频的画面或播放能力。

左手区形成一个连贯的簇：`Q`/`E` 换视频，`A`/`D` 小幅 seek，`W`/`S` 大幅 seek，全部单手可达。

菜单栏的“播放”菜单显示播放/暂停 `Space`、显示比例、顺时针旋转 `R`、水平翻转 `H` 与可勾选的逐帧模式 `G`；“显示”菜单显示素材浏览区 `Tab`、搜索 `Cmd+F`、铺满 / 恢复 `F`（鼠标中键同样可用）与全屏。既有的 `A` / `D` / `W` / `S` 与方向键跳转继续由窗口收键器处理，文本输入获得焦点时不抢占输入。

开着逐帧模式时，这两组前后跳转的键换算成帧：`A`/`←` 和 `D`/`→` 一次一帧，`W`/`↑` 和 `S`/`↓` 一次十帧。其余键位不变。
...
│   └── FrameTimeline.swift          # 逐帧时间表（ffprobe 读包，不解码）
│
├── Media/
│   ├── MediaItem.swift
