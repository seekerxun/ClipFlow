...
### 窗口缩放与全屏性能（代码已优化，待真机手测）

现已合并 mpv 新画面与窗口尺寸变化产生的重复更新，主线程最多保留一个待处理绘制；连续缩放和全屏过渡期间约以 30fps 更新画面，结束后补画最新一帧，音频链路不参与限频。合并与限频逻辑已有自动测试，但 1080p / 高码率 4K 的主线程占用、实际观感和声音连续性仍需真机手测，不能写成已验收。
...
- **视频画面不得被播放列表覆盖**
- **播放控制条固定在视频下方的独立区域，不得覆盖视频画面**
- 素材浏览区收起时，只在窗口标题栏右侧保留一个紧凑的恢复按钮，不得覆盖视频画面
- 界面采用沉浸式深色外观；标题栏、素材浏览区和播放控制条使用 macOS 原生半透明材质，正文区域保持足够对比度
...
- **启动时列表为空**，不自动打开上次的文件夹或文件。素材靠拖入、`Cmd+O`，或 Finder 里用「打开方式」加入
- Finder 右键「打开方式」可选片巡。当前窗口若是空列表，视频加进这一扇；若已经有内容，再开一扇新窗口。菜单「新建窗口」（`Cmd+N`）也可多开
- 打开文件夹（`Cmd+O`）：把该目录扫到的视频**加入当前窗口的列表**，不清空已有条目；可选多个文件夹
- 可拖入多个文件夹、多个视频，或二者混合；能作为视频的都加入**当前窗口**列表（追加），同一路径已在列表中则不加第二次；非视频文件安静跳过
- 列表不显示单一目录名；数量、排序、列表/网格切换仍在
- 搜索框默认隐藏；按 `Cmd+F` 时自动显示素材浏览区、展开搜索框并立即取得输入焦点。搜索中按 `Esc`：有内容时先清空，再按一次退出搜索
...
- 播放 / 暂停
- 拖动进度
- 音量、静音；在**播放区**用鼠标滚轮调音量（上大下小），素材列表上的滚轮仍滚动列表；音量记住最后一次调整并在下次启动时恢复
- 倍速
- 循环播放（单个循环 / 列表循环 / 关闭）；循环模式记住最后一次选择并在下次启动时恢复
...
关闭事件现已绑定到具体窗口，并按“先释放渲染上下文、再销毁 mpv”的顺序清理；代码路径与重复关闭已覆盖，普通窗口、全屏窗口和多窗口的实际操作仍需真机手测。
...
ffprobe 元数据、自动封面、`B` 键指定封面和 24 帧精灵图回退均已接入，并覆盖超时、取消、并发上限和失败落库。自动测试使用生成素材覆盖了回退与输出规格；真实旧格式 MKV / AVI / RM 仍待人工素材验收。极低帧率或实际帧数不足 24 的素材会重复映射时间上最近的真实帧，时间戳仍记录该真实帧；系统仍要求通过 Homebrew 安装 ffmpeg。
...
│   ├── FFmpegSpriteGenerator.swift  # ffmpeg 单进程封面 / 精灵图回退
