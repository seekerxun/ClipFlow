...
- 控制条只保留播放、逐帧时的前后帧、音量、倍速、显示比例、旋转与循环；逐帧模式和全屏在菜单栏操作
...
- 倍速
- 显示比例覆盖（原始、1:1、4:3、3:2、16:9、9:16），切换时即时重绘，不重新载入视频、不增加滤镜
- 画面顺时针旋转，每次 90°，按 0° / 90° / 180° / 270° 循环，不重新载入视频、不增加滤镜
...
菜单栏的“播放”菜单显示播放/暂停 `Space` 与可勾选的逐帧模式 `G`；“显示”菜单显示素材浏览区 `Tab`、搜索 `Cmd+F`、铺满 / 恢复 `F`（鼠标中键同样可用）与全屏。既有的 `A` / `D` / `W` / `S` 与方向键跳转继续由窗口收键器处理，文本输入获得焦点时不抢占输入。
