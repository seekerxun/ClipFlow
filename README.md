...
- 控制条只保留播放、逐帧时的前后帧、音量、倍速与循环；显示比例、旋转、逐帧模式和全屏在菜单栏操作
...
菜单栏的“播放”菜单显示播放/暂停 `Space`、显示比例、顺时针旋转 `R` 与可勾选的逐帧模式 `G`；“显示”菜单显示素材浏览区 `Tab`、搜索 `Cmd+F`、铺满 / 恢复 `F`（鼠标中键同样可用）与全屏。既有的 `A` / `D` / `W` / `S` 与方向键跳转继续由窗口收键器处理，文本输入获得焦点时不抢占输入。
